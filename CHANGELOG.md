...
- User prompt context injection: injects the latest user question (truncated to 800 characters) into the rewrite system prompt for better pronoun resolution and topical accuracy, matching upstream `claudish-to-english`.
- Unit test suite covering prompt generation, text extraction, session history traversal, and extension registration using `bun:test`.
